...
def write_reports(out, summary, recoveries):
    runtime_variant = 'runtime_variant' in read(out/'benchmark_manifest.json')
    report_root = ROOT if out.resolve() == OUT.resolve() else out
...
'''
    if runtime_variant:
        benchmark = benchmark.replace('The accepted Haskell/PyTorch core was not edited.',
            'The Haskell core and recognition objectives are unchanged. This separate runtime variant enables automatic CUDA/CPU placement; its parent manifest identifies the historical CPU run.')
    (report_root/'CONTROLLED_BENCHMARK_REPORT.md').write_text(benchmark, encoding='utf8')
    ...
    interpretation_start = len(lines)
    ...
    if runtime_variant:
        # The prose above describes the completed historical CPU run, not an
        # arbitrary future device-runtime run. Keep only newly computed tables.
        lines = lines[:interpretation_start] + ['', 'This is a separate runtime variant. Interpret its newly computed tables and JSON; historical CPU point estimates are not copied into this run.']
    (report_root/'CONTROLLED_EXPERIMENT_REPORT.md').write_text('\n'.join(lines)+'\n', encoding='utf8')
