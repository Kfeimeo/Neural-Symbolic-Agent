"""Render the audited partial-study summary, with comparable MDL boundaries."""
import json
...
def pct(x):
    return '未测' if x is None else f'{100*x:.2f}%'


def render(e=None):
    e = e or json.loads((OUT/'completed_evidence.json').read_text(encoding='utf8'))
    arms = e['replay']['arms']
    c = e['coverage']
    text = [
        ...
        '| 实验部分 | 已完成 / 预定 | 可支持的结论 |',
        '|---|---:|---|',
        f'| medium 固定-frontier 重压缩 | {c["replay_records"]}/36 | 3 benchmark seeds × 3 training seeds × 4 arms 的配对诊断 |',
        f'| 六轮 EC runs | {c["completed_runs"]}/{c["planned_runs"]} | 仅 seed 101 / medium / training seed 1 的四臂 |',
        f'| training-round records | {c["training_round_records"]}/{c["planned_training_round_records"]} | 上述四臂均完成六轮 |',
        f'| held-out records | {c["held_out_records"]}/{c["planned_held_out_records"]} | B0/B1 的 library 1–6 轮、recognition 1–5 轮 |', '',
    ...
    for arm, r in arms.items():
        values = [pct(r['er'][kind][f'ER@{n}']) for kind in ('syntactic', 'behavioural', 'equivalence_usable') for n in (1, 2)]
        text.append(f"| {arm} | {' | '.join(values)} |")
    text += ['', 'ER@k 表示 active latent 获得至少 k 个不同训练 task 的支持，不是 frontier 排名。syntactic 使用冻结的 β-normal rewrite 判据；behavioural 是有限 probes 上的子表达式见证；usable 是当前显式 E/R 空间中能够缩短程序的 latent rewrite。三者的搜索空间不同，不保证集合严格嵌套。', '',
    ...
    for arm, r in arms.items():
        ...
        values = [r['exact_syntactic_recall'], r['beta_recall'], b['precision'], b['recall'], b['f1'], r['parameterised_latent_recall']]
        text.append(f"| {arm} | {' | '.join(map(pct, values))} |")
    text += ['', '**四臂条件参数恢复率均为 2/20 = 10%。** 参数化 recall 均为 2/36 = 5.56%。条件分母逐 latent 检查多个有效参数值是否真的暴露；generalised recovery 要求参数类型兼容、并通过该 latent 全部有效参数值的 probes。', '',
             ...
             '|---|---:|---:|---:|---:|']
    for arm, r in arms.items():
        text.append(f"| {arm} | {r['new_inventions_total']} | {r['final_invention_count_mean']:.3f} | {r['extra_delta_mdl_mean']:+.6f} | {r['raw_to_final_delta_mdl_mean']:+.6f} |")
    text += ['', '正 ΔMDL 表示 MDL 下降、拟合改善。局部列从表示转换后的 corpus 开始计算；总列使用相同边界：原始 corpus 经冻结 inside-outside 拟合得到的 MDL，减去最终 grammar/corpus 的 MDL。原结果字段为 `delta_mdl` 与 `total_delta_mdl`（B0 二者边界相同）。', '',
             ...
             '|---|---:|---:|---|---:|---:|']
    for r in e['final_runs']:
        b = r['recovery']['metrics']['behavioral']
        text.append(f"| {r['arm']} | {r['solved']}/{r['tasks']} ({pct(r['solved']/r['tasks'])}) | {r['invention_count']} | "
                    f"{' / '.join(pct(b[k]) for k in ('precision','recall','f1'))} | {pct(r['parameterised_recall'])} | {r['cumulative_raw_to_final_delta_mdl']:+.6f} |")
    text += ['', 'B0/B1/B3 均恢复 F04，B2 没有恢复目标 latent。B3 相对于 B2 在这个 cell 中保留了 recovery，但未超过 B0，且少解出一个训练任务。这支持继续检查 canonical representation 的信息损失，不能证明 e-class AU 普遍必要。', '',
             '| Arm | syntactic ER@1 / ER@2 | behavioural ER@1 / ER@2 | usable ER@1 / ER@2 |',
             '|---|---|---|---|']
    for r in e['final_runs']:
        if r['metrics'] is None:
            text.append(f"| {r['arm']} | 完整 exposure 汇总未取得 | 完整 exposure 汇总未取得 | 闭包超过 512；未测 |")
        else:
            vals = [' / '.join(pct(r['metrics']['er'][kind][f'ER@{n}']) for n in (1, 2)) for kind in ('syntactic','behavioural','equivalence_usable')]
            text.append(f"| {r['arm']} | {' | '.join(vals)} |")
    text += ['', 'B2 的 recovery 与 parameter recall 是独立计算的，不依赖饱和，因此表中的 0 是实测值；其失败的 exposure 项不是 0。', '',
             ...
             '|---|---:|---:|---:|---:|---:|---|']
    for row in e['evaluation']:
        if row['iteration'] != 6 or row['mode'] != 'library':
            continue
        s = row['summary']
        vals = [pct(s['curve'][str(n)]) for n in (100,300,1000,3000,10000)]
        text.append(f"| {row['arm']} | {' | '.join(vals)} | {s['mean_first_solution_nodes_solved']:.2f} / {s['median_first_solution_nodes_solved']:.2f} |")
    text += ['', '两臂 S(10000) 都为 20%；较低预算曲线有差异。solved-only rank 只描述已解子集，不能替代完整 solve curve。逐任务 rank 和全部 22 条中间轮次曲线保存在 `completed_evidence.json`，包括未解的 null；没有用第五轮 recognition 结果替代缺失的第六轮结果。', '',
             '## 5. 五个主要问题的回答', '',
